...
export const Text_Edit: t_signatures.Text_Edit = ($) => _p.deprecated_cc($, ($): t_out.Text_Edit => {
    switch ($[0]) {
        case 'replace': return _p.ss($, ($) => ['replace', Replace(
            $
        )])
        case 'delete': return _p.ss($, ($) => ['delete', ({
            'range': _p.deprecated_cc($['range'], ($) => Range(
                $
            )),
        })])
        case 'insert': return _p.ss($, ($) => ['insert', ({
