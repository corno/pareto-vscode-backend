...
export const Diagnostic_Severity: t_signatures.Diagnostic_Severity = ($) => _p.deprecated_cc($, ($): t_out.Diagnostic_Severity => {
    switch ($[0]) {
        case 'error': return _p.ss($, ($) => ['error', null])
        case 'warning': return _p.ss($, ($) => ['warning', null])
        case 'information': return _p.ss($, ($) => ['information', null])
        case 'hint': return _p.ss($, ($) => ['hint', null])
